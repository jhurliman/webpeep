...
export default class StateSound extends ISound {
  constructor(audioCtx, libraryEntry) {
    ...
    this.segments = Array.apply(null, Array(count)).map((_, i) => {
      const baseUrl = libraryEntry.baseUrl
      // {baseUrl}{name}.{nn}{fileExtension}
      const url = `${baseUrl}${this.name}.${Utils.zeroPad(i + 1, 2)}${ext}`
  ...
  doPlay(intensity, hash) {
    ...
    if (!curSource) {
      ...
      if (!curSegment)
        return console.warn(`[WARN] StateSound ${this.name} has no segments`)
      ...
      sharedGain.linearRampToValueAtTime(
        intensity,
        curTime + StateSound.VOLUME_CHANGE_TIME
      )
    }
    ...
    const offset = Math.max(
      0.1,
      curSource.endTime - curTime - StateSound.FADE_TIME
    )
  ...
  playWithCrossfade(segment, offset, gain) {
    ...
    source.songbirdSource = window.songbird.createSource(songbirdOpts)
    gainNode.connect(source.songbirdSource.input)
